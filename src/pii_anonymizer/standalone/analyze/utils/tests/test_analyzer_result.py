...
class TestAnalyzerResult(TestCase):
    ...
    def test_inequality(self):
        ...
        self.assertNotEqual(
            AnalyzerResult("text", "type", 0, 10), AnalyzerResult("text", "type", 1, 10)
        )
        self.assertNotEqual(
            AnalyzerResult("text", "type", 0, 10), AnalyzerResult("text", "type", 0, 11)
        )

    def test_repr(self):
        expected = "Text sample_data at position (0,10) was identified as type"
        self.assertEqual(
            AnalyzerResult("sample_data", "type", 0, 10).__repr__(), expected
        )
