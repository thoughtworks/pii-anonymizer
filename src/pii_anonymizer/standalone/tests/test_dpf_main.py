...
from src.pii_anonymizer.standalone.constants import ACQUIRE, REPORT
from src.pii_anonymizer.standalone.dpf_main import DPFMain
...
class TestDPFMain(TestCase):
    ...
    @patch("src.pii_anonymizer.standalone.write.csv_writer.CsvWriter.write_csv")
    @patch("src.pii_anonymizer.standalone.write.csv_writer.CsvWriter.__init__")
    @patch(
        "src.pii_anonymizer.standalone.report.report_generator.ReportGenerator.generate"
    )
    @patch(
        "src.pii_anonymizer.standalone.report.report_generator.ReportGenerator.__init__"
    )
    @patch(
        "src.pii_anonymizer.standalone.analyze.detectors.pii_detector.PIIDetector.analyze_data_frame"
    )
    @patch("src.pii_anonymizer.standalone.acquire.csv_parser.CsvParser.parse")
    @patch("src.pii_anonymizer.standalone.acquire.csv_parser.CsvParser.__init__")
    def test_run_parses_the_config_file_and_invokes_respective_stages_correctly(
        ...
        mock_csv_writer_write_csv.assert_called_with(
