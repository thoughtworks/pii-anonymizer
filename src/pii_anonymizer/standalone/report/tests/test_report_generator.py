...
from src.pii_anonymizer.standalone.report.report_generator import ReportGenerator
from src.pii_anonymizer.standalone.analyze.utils.analyzer_result import AnalyzerResult


class TestReportGenerator(TestCase):
    @patch(
        "src.pii_anonymizer.standalone.report.report_generator.ReportGenerator.setup_logging_config"
    )
    ...
    def test_high_level_reporting_returns_columns_with_PII_values_when_given_a_results_data_frame(
        ...
        self.assertCountEqual(
            expected_data_frame,
            self.report_generator_high_level.generate_report_content(result_data_frame),
        )
    ...
    def test_medium_level_reporting_returns_data_frame_with_detectors_and_column_details(
        ...
        expected_data_frame = pd.DataFrame(
            {
                "summary": pd.Series({"NRIC": (1, "50%"), "EMAIL": (1, "50%")}),
    ...
    def test_calculate_detector_stats_returns_detector_counts_and_percentages(self):
        ...
        actual_result = (
            self.report_generator_medium_level.calculate_detector_stats_for_each_column(
    ...
    @patch("logging.info")
    @patch(
        "src.pii_anonymizer.standalone.report.report_generator.ReportGenerator.generate_report_content"
    )
