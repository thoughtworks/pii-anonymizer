...
from src.pii_anonymizer.spark.constants import LOCATION, REPORT_LEVEL
...
class ReportGenerator:
    ...
    def spark_generate_medium_level_report(self, results_df: DataFrame) -> pd.DataFrame:
        columns = results_df.columns
        detector_results = (
            results_df.rdd.flatMap(lambda row: self._get_detector_results(row, columns))
            .reduceByKey(lambda acc, next: acc + next)
            .collect()
        )
        ...
        for column in columns:
            ...
            pd_columns.append(
                pd.Series(data=detection_stats, index=report_detectors, name=column)
            )
        report_df = pd.concat(pd_columns, axis=1).fillna(0)
        return report_df

    def __get_detection_stats(
        self,
        column: list,
        report_detectors: list,
        ...
        for detector in report_detectors:
            column_detector_count = next(
                filter(
                    lambda result: result[0] == (column, detector), detector_results
                ),
                default_value,
            )
            if len(column_detector_count) > 0:
                count = column_detector_count[1]
                percentage_value = self.__calculate_percentage(
                    item_count=count, total_count=num_rows
                )
    ...
    def is_empty_report_dataframe(self, results_df: DataFrame) -> bool:
        if self.dataframe_is_empty == None:
            self.dataframe_is_empty = results_df.rdd.flatMap(
                lambda row: self._row_is_empty_list(row)
            ).reduce(lambda acc, item: acc and item)
